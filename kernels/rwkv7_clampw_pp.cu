...
__global__ void forward_kernel(int T,int H,F_ r_,F_ w_,F_ k_,F_ v_,F_ a_,F_ b_,bf* __restrict__ y_,float* s__,float* __restrict__ sa_)
{
    ...
    float* __restrict__ s_ = s__ + i64(bb*H+hh) * i64((T/64)*N*N);
    float state[N];
#pragma unroll
    for (int j=0; j<N; ++j) {
        state[j] = 0.0f;
...
{
    forward_kernel<64><<<dim3(H,B),dim3(64)>>>(T,H,r,w,k,v,a,b,y,s,sa);
}
...
{
    assert(T%64 == 0);
    backward_kernel<64><<<dim3(H,B), dim3(64)>>>(T,H,r,w,k,v,a,b,dy,s,sa,dr,dw,dk,dv,da,db);
}
