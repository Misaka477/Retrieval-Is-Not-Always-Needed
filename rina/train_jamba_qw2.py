"""Train RINA-Jamba-QW2: Q4_0 block-wise 全量化（含 embedding/head）
从现有 QW checkpoint 加载权重（state_dict key 完全兼容）。
"""
import os, math, time, argparse
import torch, torch.nn.functional as F, numpy as np
from tqdm import tqdm
from rina.model_jamba_qw2 import RINA_Jamba_QW2, QW2_Config

device = 'cuda' if torch.cuda.is_available() else 'cpu'
BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def train(cfg, steps=50000, lr=1e-3, bsz=1, acc=2, out='models/out-rina-jamba-qw2', resume=None):
    ...
    model = RINA_Jamba_QW2(cfg).to(device)
    start_step = 0
    ...
    for step in pbar:
        ...
        if step > 0 and step % 500 == 0:
            torch.save({'model': model.state_dict(), 'step': step}, f'{out}/jambaqw2_{step}.pt')
            keep = sorted([f for f in os.listdir(out) if f.startswith('jambaqw2_') and f.endswith('.pt')],
                          key=lambda x: int(x.split('_')[-1].split('.')[0]))
            for f in keep[:-4]: os.remove(os.path.join(out, f))

    log_file.close()
    torch.save({'model': model.state_dict()}, f'{out}/jambaqw2_final.pt')
...
if __name__ == '__main__':
    ...
    p.add_argument('--out', type=str, default='models/out-rina-jamba-qw2')
    p.add_argument('--resume', type=str, default=None)
    p.add_argument('--weight_bits', type=int, default=4)
    p.add_argument('--ssm_q', type=int, default=4)
    args = p.parse_args()
    cfg = QW2_Config(vocab_size=128256, block_size=args.seq, use_int4=True,
