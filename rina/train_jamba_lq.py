"""Train RINA-Jamba-LQ: log-space SSM + KV 量化对比"""
...
def train(cfg, steps=50000, lr=1e-3, bsz=2, out='models/out-rina-jamba-lq', resume=None):
    ...
    model = RINA_Jamba_LQ(cfg).to(device)
    start_step = 0
    ...
    if resume:
        ...
        n1 = load_path(os.path.join(BASE, 'models/out-0.1b-c-distil-multistep/c_final.pt'),
                       'inertia.', 'path.', path_only=False)
        n2 = load_path(os.path.join(BASE, 'models/out-0.1b-a-v2/a_final.pt'),
                       'attn.', 'path.', path_only=False)
        n3 = load_path(os.path.join(BASE, 'models/out-0.1b-a-v2/a_final.pt'),
                       'NONEXIST', '', path_only=False)
        print(f'  Loaded: SSM={n1} Sparse={n2} Shared={n3}')
    ...
    for step in pbar:
        ...
        if step % 500 == 0:
            torch.save({'model': model.state_dict(), 'step': step}, f'{out}/jambalq_{step}.pt')
            keep = sorted([f for f in os.listdir(out) if f.startswith('jambalq_') and f.endswith('.pt')],
                          key=lambda x: int(x.split('_')[-1].split('.')[0]))
            for f in keep[:-4]: os.remove(os.path.join(out, f))

    log_file.close()
    torch.save({'model': model.state_dict()}, f'{out}/jambalq_final.pt')
...
if __name__ == '__main__':
    ...
    p.add_argument('--out', type=str, default='models/out-rina-jamba-lq')
    p.add_argument('--resume', type=str, default=None)
    p.add_argument('--ssm_q', type=int, default=4, help='SSM quant bits: 0=fp32 8=q8 4=q4')
    args = p.parse_args()
    cfg = RJLQ_Config(vocab_size=128256, block_size=args.seq, use_int4=True,
                      n_embd=640, n_layer=16, n_head=10, n_kv_heads=5,
                      d_c=160, head_dim=64,
                      sparse_k=16, sparse_window=32, sparse_local_w=4,
                      ssm_steps=3, quant_mode='q4k_q2v',
                      ssm_qbits=args.ssm_q)
