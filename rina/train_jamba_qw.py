"""Train RINA-Jamba-QW: q4 权重 + LSC q4 SSM + q2+q1 KV"""
...
def train(cfg, steps=50000, lr=1e-3, bsz=2, out='models/out-rina-jamba-qw', resume=None):
    ...
    model = RINA_Jamba_QW(cfg).to(device)
    start_step = 0
    ...
    print(f'Full training (weight_bits={cfg.weight_bits}): {trainable} params')
    ...
    for step in pbar:
        ...
        if step % 500 == 0:
            torch.save({'model': model.state_dict(), 'step': step}, f'{out}/jambaqw_{step}.pt')
            keep = sorted([f for f in os.listdir(out) if f.startswith('jambaqw_') and f.endswith('.pt')],
                          key=lambda x: int(x.split('_')[-1].split('.')[0]))
            for f in keep[:-4]: os.remove(os.path.join(out, f))

    log_file.close()
    torch.save({'model': model.state_dict()}, f'{out}/jambaqw_final.pt')
...
if __name__ == '__main__':
    ...
    p.add_argument('--out', type=str, default='models/out-rina-jamba-qw')
    p.add_argument('--resume', type=str, default=None)
    p.add_argument('--weight_bits', type=int, default=4)
    p.add_argument('--ssm_q', type=int, default=4)
    args = p.parse_args()
    cfg = QW_Config(vocab_size=128256, block_size=args.seq, use_int4=True,
