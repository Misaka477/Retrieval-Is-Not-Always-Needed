...
def q4_0_block(x, gs=32):
    """Q4_0 block-wise quantization: block of 32, symmetric, fp16 scale
       匹配推理引擎 .rinn 的 Q4_0 存储格式。
    """
...
class Q4BlockLinear(nn.Module):
    """线性层，权重按 Q4_0 block-wise 量化（匹配推理引擎格式）"""
...
class InertiaLayerQW2(nn.Module):
    def __init__(self, c):
        super().__init__()
        ...
        self.w_mem = nn.ModuleList([
            Q4BlockLinear(self.d_c, self.n_head*self.d_h, c.bias, wb)
            for _ in range(self.ssm_steps)])
        self.w_decay = nn.ModuleList([
            Q4BlockLinear(self.d_c, self.n_head, c.bias, wb)
            for _ in range(self.ssm_steps)])
    ...
    def _ssm_scan(self, mem, decay, qbits):
        ...
        lca = torch.cumsum(a.clamp(min=1e-38).log(), dim=1)
        ca = lca.exp()
...
class QW2Block(nn.Module):
    def __init__(self, c, layer_type, layer_idx=0):
        ...
        self.mlp = SwiGLU(c)
        if layer_type == 0: self.path = InertiaLayerQW2(c)
        else: self.path = MLALayerQW2(c, sparse=True)
...
class RINA_Jamba_QW3(nn.Module):
    def __init__(self, c):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h=nn.ModuleList([QW2Block(c, ltypes[i], i) for i in range(c.n_layer)]),
