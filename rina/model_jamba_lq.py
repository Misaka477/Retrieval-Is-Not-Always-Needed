"""RINA-Jamba-LQ: SSM 的 cumprod 改为 log-space cumsum, 中间结果可压到 q8"""
import math
...
class RJLQ_Config:
    ...
    ssm_steps: int = 3
    quant_mode: str = 'q4k_q2v'
    ssm_qbits: int = 0  # 0=fp32, 8=q8, 4=q4 SSM 中间量化
...
class InertiaLayerLQ(nn.Module):
    """SSM with log-space cumsum — 中间结果可压 q8/q4"""
    def __init__(self, c):
        super().__init__()
        self.d_c = c.d_c; self.n_head = c.n_head; self.d_h = c.head_dim
        self.ssm_qbits = getattr(c, 'ssm_qbits', 0)
        ...
        self.w_mem = nn.ModuleList([
            nn.Linear(self.d_c, self.n_head * self.d_h, bias=c.bias)
            for _ in range(self.ssm_steps)])
        self.w_decay = nn.ModuleList([
            nn.Linear(self.d_c, self.n_head, bias=c.bias)
            for _ in range(self.ssm_steps)])
        self.w_out = nn.Linear(self.n_head * self.d_h + c.n_embd, c.n_embd, bias=c.bias)
        self.resid_drop = nn.Dropout(c.dropout)

    def _ssm_scan(self, mem, decay, qbits=None):
        """log-space cumsum: ca = exp(cumsum(log(decay)))
           qbits 控制中间量量化位数: None=fp32, 8=q8, 4=q4"""
        B = decay.size(0); T = decay.size(1)
        d = decay.squeeze(-1) if decay.dim() > 3 else decay  # [B,T,H]
        a = d.unsqueeze(-1).expand(-1, -1, -1, self.d_h)
        lca = torch.cumsum(a.log(), dim=1)
        ca = lca.exp()
        sf = ca * torch.cumsum(mem / (ca + 1e-8), dim=1)
        if qbits is not None:
            for name, t in [('lca',lca),('ca',ca),('sf',sf)]:
    ...
    def forward(self, x, k=-1):
        ...
        if k < 0:
            ...
            decays = [torch.sigmoid(self.w_decay[i](cq)).view(B, T, self.n_head, 1)
                      for i in range(self.ssm_steps)]
...
class RINA_Jamba_LQ(nn.Module):
    def __init__(self, c):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h=nn.ModuleList([RJLQBlock(c, ltypes[i], i) for i in range(c.n_layer)]),
        ...
        print(f'RINA_Jamba_LQ: {core/1e6:.2f}M core | {sum(p.numel() for p in self.parameters())/1e6:.2f}M total')
