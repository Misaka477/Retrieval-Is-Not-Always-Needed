"""Minimal SFT: 200 best Magicoder + 200 OASST, loss mask, smaller dataset."""
...
def load_best(path, n=200, max_seq=512):
    """Load top-N examples with longest completions."""
    rows = []
    with open(path) as f:
        for line in f:
            d = json.loads(line)
            ...
            c_ids = tok.encode(comp)
            if 10 < len(p_ids) + len(c_ids) <= max_seq and len(c_ids) > 10:
                rows.append((len(c_ids), p_ids, c_ids))
    rows.sort(reverse=True)  # longest first
    return [(p, c) for _, p, c in rows[:n]]

def load_oasst(path, n=200, max_seq=300):
    ...
    for tree in trees.values():
        ...
        p = f"User: {root['text']}\nAssistant: "
        c = ' ' + asst['text']
        p_ids = tok.encode(p); c_ids = tok.encode(c)
        if 10 < len(p_ids) + len(c_ids) <= max_seq and len(c_ids) > 10:
            rows.append((len(c_ids), p_ids, c_ids))
        if len(rows) >= n: break
    return [(p, c) for _, p, c in rows]
...
def train(steps=5000, lr=3e-5):
    magic = load_best(os.path.join(BASE, 'data/magic_oss_en.jsonl'), 200)
    oasst = load_oasst(os.path.join(BASE, 'data/oasst1/train.jsonl'), 200)
    rows = magic + oasst
    print(f'Mini SFT: {len(magic)} Magic + {len(oasst)} OASST = {len(rows)} total')
    ...
    print('Loaded Phase 1 checkpoint (clean base, no prior SFT)')
    
    out = os.path.join(BASE, 'models/out-jamba-sft-mini')
    ...
    torch.save({'model': model.state_dict(), 'step': steps}, f'{out}/sft_mini_final.pt')
    log_file.close()
    print('Done')
