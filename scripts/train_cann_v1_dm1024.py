"""v1-style dense gate + dm=1024 full training."""
...
from modules.cann_ssm import RINASeqModel, _full_forward, _cell_full, _cell_ssm
from tqdm import tqdm

device = "cuda"; torch.manual_seed(42)

DM, NP, SEQ, BS, AE = 1024, 4096, 64, 16, 4
EPOCHS, LR = 10, 3e-4
N_SEGMENTS = 200000; SUBSAMPLE = 8
CKPT_NAME = "cann_v1_dm1024"
...
print(f"Model (dense gate, dm={DM}, ae={AE})...", flush=True)
    ...
    for b in pbar:
        ...
        if b > 0 and b % NIAH_EVERY == 0:
            ...
            with torch.autocast("cuda", dtype=torch.float16):
                ...
                h = model.cell(h + i_ext, emb[:, -1], step=x_niah.shape[1] - 1)
                logits.append(model.head(model.state_norm(h)))
                out_n = torch.stack(logits, dim=1)
