...
def _load_wkv7():
    ...
    _WKVC = load(name="rwkv7_clampw",
        ...
        extra_cuda_cflags=["-D_N_=64", "-O3"], is_python_module=True, verbose=False)
    # Register for torch.ops access
    torch.ops.rwkv7_clampw.forward = _WKVC.forward
    torch.ops.rwkv7_clampw.backward = _WKVC.backward
...
class WKV7Fn(torch.autograd.Function):
    ...
    def forward(ctx, r, w, k, v, a, b, s=None, sa=None):
        ...
        _WKVC.forward(r.contiguous(),w.contiguous(),k.contiguous(),v.contiguous(),
    ...
    def backward(ctx, dy):
        ...
        _WKVC.backward(r.contiguous(),w.contiguous(),k.contiguous(),v.contiguous(),
