"""MoHE with FFN experts (RWKV-style), per-token routing, depth chain, consolidate."""
...
# ── FWKV-style FFN expert ──
class FFNExpert(nn.Module):
    """Standard 4× FFN with squared ReLU activation (RWKV channel-mixing style)."""
    def __init__(self, dm):
        super().__init__()
        self.key = nn.Linear(dm, dm * 4)
        self.value = nn.Linear(dm, dm * 4)
        self.out = nn.Linear(dm * 4, dm)
    def forward(self, h):
        k = torch.relu(self.key(h))
        v = self.value(h)
        return self.out(k * v)

class MoHEFFN(nn.Module):
    """MoHE with FFN experts, per-token routing, depth chain, consolidate."""
    def __init__(self, vocab, dm, n_experts=4, aux_loss_weight=0.1, route_noise=0.0, topk=0, wkv_no_grad=False, soft_routing=False):
        super().__init__()
        ...
        # FFN experts (no gate, no patterns, no attractor)
        self.experts = nn.ModuleList([FFNExpert(dm) for _ in range(n_experts)])

        # Selective SSM (data-dependent decay)
        self.dt_linear = nn.Linear(dm, dm)
        self.dt_linear.bias.data.fill_(0.5)
    ...
    def forward(self, x, wkv_state=None):
        ...
        dt = F.softplus(self.dt_linear(emb))
        w4d = torch.exp(-dt.view(B, T, H, N)).contiguous()
        ...
        for depth in range(3):
            ...
            # FFN experts: no x_emb needed (unlike attractor)
