"""
Code continued training — test cross-distribution scaling.
Loads from latest FineWeb checkpoint, trains on StarCoder (Python subset).
If ppl drops significantly below FineWeb plateau (~48), architecture 
truly benefits from distribution diversity, not just data volume.
"""
...
# Load from latest FineWeb checkpoint
print("Loading checkpoint...", flush=True)
sd = torch.load("checkpoints/fineweb_resume.pt", map_location=device, weights_only=False)
...
# Tokenize code data
CODE_CACHE = os.path.join(CKPT_DIR, "code_tokens.pt")
if os.path.exists(CODE_CACHE):
    ...
    print("Loading StarCoder Python...", flush=True)
...
for ep in range(start_ep, EPOCHS + 1):
    ...
    log_file = os.path.join(CKPT_DIR, f"code_ep{ep}_log.csv")
