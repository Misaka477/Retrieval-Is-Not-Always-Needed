"""
Code seq=128 progressive training — load from code checkpoint.
"""
...
MAX_TOKENS = 200_000_000; CKPT_NAME = "code_seq128"
...
# Load from code checkpoint
print("Loading code checkpoint...", flush=True)
src = "checkpoints/rine_code_ep1_latest.pt"
if not os.path.exists(src):
    src = "checkpoints/code_ep1_latest.pt"
...
if os.path.exists(CODE_CACHE):
    ...
    print(f"Tokenizing (seq={SEQ}, limited to ~200M tokens)...", flush=True)
...
print(f"VRAM test: seq={SEQ} bs={BS}", flush=True)
try:
    x = torch.randint(0, V, (BS, SEQ), device=device)
    with torch.amp.autocast("cuda"): _ = model(x)
    torch.cuda.synchronize()
    print(f"  VRAM OK (~{torch.cuda.memory_allocated()/1024**3:.1f}GB)", flush=True)
except RuntimeError as e:
    print(f"  OOM: {e}", flush=True)
    BS = 4; print(f"  Falling back to bs={BS}", flush=True)

# Warmup: 3 steps on real training data
...
for ep in range(start_ep, EPOCHS + 1):
    ...
    log_file = os.path.join(CKPT_DIR, f"code_seq128_log.csv")
    if not os.path.exists(log_file):
        with open(log_file, "w") as f:
            f.write("step,ppl,att_pct,dead_pct,norms_mu,cos_drift,frob_drift,r95,lr\n")
    for bi in pbar:
        train_steps += 1; total_loss = total_loss
        ...
        if train_steps % 200 == 0:
            ...
            norms = P.norm(dim=-1); dead = (norms < 0.01).float().mean().item(); norms_mu = norms.mean().item()
