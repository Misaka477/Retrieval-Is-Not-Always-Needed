# -*- coding: utf-8 -*-
"""CANN-SSM LM on TinyStories. Causal, no attention.""" 
...
print(f"2: {len(texts):,} stories", flush=True)
...
for ep in range(EPOCHS):
    ...
    if ep % 5 == 4 or ep == EPOCHS-1:
        torch.save(m.state_dict(), f"D:\\Software_Development\\Project\\RINA_Project\\checkpoints\\cann_ep{ep+1}.pt")
        m.eval()
        prompt = "password "
        ...
        with torch.no_grad():
            for _ in range(40):
                nid = m(xg, use_cuda_seq=True)[0, -1].argmax().item()
                xg = torch.cat([xg, torch.tensor([[nid]], device=device)], dim=1)
        print(f"  gen: {tok.decode(xg[0].tolist())[:120]}", flush=True)

torch.save(m.state_dict(), f"D:\\Software_Development\\Project\\RINA_Project\\checkpoints\\cann_final.pt")
