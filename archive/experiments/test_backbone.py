"""Test backbone output at different temperatures and seeds."""
...
for i in range(12):
    ...
    for k in ['w0','w1','w2','a0','a1','a2','v0','v1','v2','g1','g2','k_k','k_a','r_k']: getattr(li,k).data.copy_(sd[b+f'att.{k}'])
...
for temp in [0.01, 0.5, 0.8, 1.0]:
    for seed in [42, 123, 999]:
        torch.manual_seed(seed); random.seed(seed); torch.cuda.manual_seed(seed)
        g=p.clone()
        with torch.no_grad():
            for _ in range(32):
                l,h=bb(g)
                if temp<0.1: g=torch.cat([g,l[:,-1].float().argmax(-1,keepdim=True)],1)
                else: g=torch.cat([g,torch.multinomial(torch.softmax(l[:,-1]/temp,-1),1)],1)
        text=tok.decode(g[0].tolist()[len(tok.encode(prompt)):])
        print(f'temp={temp} seed={seed}: {repr(text[:60])}')
print('Done.')
