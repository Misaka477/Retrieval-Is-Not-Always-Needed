"""SSM backbone + diffusion denoising — 109M scale. Run on 32G cloud GPU."""
...
SEQ = 512
LR = 2e-4
N_STEPS = 24000  # ~100M tokens: 8×512×24000 = 98M
DATA_PATH = 'checkpoints/mohe_fw_rwkv_1b.npy'
CKPT_DIR = 'checkpoints'
CSV_PATH = os.path.join(CKPT_DIR, 'ssm_diff_109m.csv')

model = MoHESSM(VOCAB, DM, NP, n_experts=N_EXP, topk=2).to(device)
print(f'109M SSM: {sum(p.numel() for p in model.parameters())/1e6:.1f}M')
...
start_step = 0
resume_path = os.path.join(CKPT_DIR, 'ssm_diff_109m_latest.pt')
...
with open(CSV_PATH, 'a' if start_step > 0 else 'w', newline='') as f:
    if start_step == 0:
        f.write('step,loss,acc,mask_rate,gn,lr\n')
...
for bi in pbar:
    ...
    if bi % 200 == 0:
        ...
        pbar.set_postfix(loss=f'{loss.item():.2f}', acc=f'{acc:.3f}',
                         mr=f'{mask_rate:.2f}', lr=f'{lr_cur:.2e}')
    if bi % 1000 == 0 or bi == N_STEPS - 1:
        lr_cur = opt.param_groups[0]['lr']
        with open(CSV_PATH, 'a', newline='') as f:
            f.write(f'{bi+1},{loss.item():.6f},{acc:.4f},{mask_rate:.4f},{gn:.1f},{lr_cur:.2e}\n')
        torch.save({'step': bi+1, 'model': model.state_dict(), 'opt': opt.state_dict()},
                   resume_path + '.tmp')
        os.replace(resume_path + '.tmp', resume_path)
        print(f'\n  Saved step {bi+1}: loss={loss.item():.3f}, acc={acc:.3f}')

torch.save({'step': N_STEPS, 'model': model.state_dict(), 'opt': opt.state_dict()},
           os.path.join(CKPT_DIR, 'ssm_diff_109m_final.pt'))
print(f'\nDone — ssm_diff_109m_final.pt')
...
with torch.no_grad():
    ...
    out = gen[0].tolist()
    uni = len(set(out))
    print(f'  Unique: {uni}/24  {"DIVERSE" if uni > 5 else "COLLAPSED"}')
    print(f'  Top tokens: {out[:8]}...')
...
with torch.no_grad():
    ...
    w = torch.exp(-dt)
    print(f'\n  dt: mean={dt.mean().item():.3f} [{dt.min().item():.4f}, {dt.max().item():.4f}]')
    print(f'  w:  mean={w.mean().item():.3f}  [{w.min().item():.6f}, {w.max().item():.6f}]')
