"""State diffuser: real data states (not AR), pure MSE (no entropy), CUDA kernel."""
...
class Cmix(nn.Module):
    def __init__(self):
        ...
        self.key=nn.Linear(C,C*4,bias=False); self.value=nn.Linear(C*4,C,bias=False)
    def forward(self,x):
...
for i in range(12):
    ...
    li.receptance.weight.data.copy_(sd[b+'att.receptance.weight']); li.key.weight.data.copy_(sd[b+'att.key.weight'])
    li.value.weight.data.copy_(sd[b+'att.value.weight']); li.output.weight.data.copy_(sd[b+'att.output.weight'])
...
opt=torch.optim.AdamW(denoiser.parameters(),lr=3e-5)

ids = torch.from_numpy(np.load('checkpoints/mohe_fw_rwkv_1b.npy', mmap_mode='r'))
BSZ, SEQ, N_STEPS = 4, 128, 50000
...
for bi in pbar:
    ...
    c_full = (torch.softmax(logits*0.05, -1) @ hd.weight)  # [B, SEQ, D]
    hp = denoiser(hn.reshape(-1,D), c_full.reshape(-1,D)).view(BSZ,SEQ,D)
    loss = F.mse_loss(hp, h.detach())  # predict h[t+1] from h[t]
    
    opt.zero_grad(); loss.backward()
    torch.nn.utils.clip_grad_norm_(denoiser.parameters(), 1.0); opt.step()
    
    if bi%2000==0:
        pbar.set_postfix(mse=f'{loss.item():.4f}')
        torch.save({'denoiser':denoiser.state_dict()}, 'checkpoints/diff_mse.pt')
...
for label,use_d in [('AR',False),('AR+Diff',True)]:
    ...
    with torch.no_grad():
        for _ in range(32):
            l,h=bb(g)
            if use_d:
                c=(torch.softmax(l*0.05,-1)@hd.weight).view(-1,D)
                hc=denoiser(h.view(-1,D),c).view(1,-1,D)
                l=hd(hc)
