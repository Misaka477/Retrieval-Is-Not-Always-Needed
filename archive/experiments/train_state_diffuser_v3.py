"""State Diffuser: official 12-layer RWKV-v7 backbone + tiny diffuser MLP."""
import sys, os; sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True'
import torch, torch.nn as nn, torch.nn.functional as F, numpy as np, types
from tqdm import tqdm

device = 'cuda'; V, D = 65536, 768

# ── Official model (exact copy from rwkv_v7_demo.py) ──
HEAD_SIZE = 64
def rwkv7_op(r,w,k,v,a,b):
    B,T,C = r.shape; H=C//HEAD_SIZE; N=HEAD_SIZE
    r=r.view(B,T,H,N).float(); k=k.view(B,T,H,N).float(); v=v.view(B,T,H,N).float()
    a=a.view(B,T,H,N).float(); b=b.view(B,T,H,N).float()
    w=torch.exp(-torch.exp(w.view(B,T,H,N).float()))
    out=torch.zeros(B,T,H,N,device=r.device); s=torch.zeros(B,H,N,N,device=r.device)
    for t in range(T):
        kk=k[:,t].view(B,H,1,N); rr=r[:,t].view(B,H,N,1)
        vv=v[:,t].view(B,H,N,1); aa=a[:,t].view(B,H,N,1)
        bb=b[:,t].view(B,H,1,N)
        s=s*w[:,t,:,None,:]+(s@aa)@bb+vv@kk
        out[:,t]=(s@rr).view(B,H,N)
    return out.view(B,T,C).to(dtype=torch.float32)

class TimeMix(nn.Module):
    def __init__(self, args, lid):
        super().__init__(); self.lid=lid; self.n_head=args.dim_att//args.head_size_a
        C=args.n_embd; H=self.n_head
        for k in ['x_r','x_w','x_k','x_v','x_a','x_g']: setattr(self,k,nn.Parameter(torch.zeros(1,1,C)))
        self.w0=nn.Parameter(torch.zeros(1,1,C)); self.w1=nn.Parameter(torch.zeros(C,64))
        self.w2=nn.Parameter(torch.zeros(64,C)); self.a0=nn.Parameter(torch.zeros(1,1,C))
        self.a1=nn.Parameter(torch.zeros(C,64)); self.a2=nn.Parameter(torch.zeros(64,C))
        self.v0=nn.Parameter(torch.zeros(1,1,C)); self.v1=nn.Parameter(torch.zeros(C,32))
        self.v2=nn.Parameter(torch.zeros(32,C)); self.g1=nn.Parameter(torch.zeros(C,128))
        self.g2=nn.Parameter(torch.zeros(128,C)); self.k_k=nn.Parameter(torch.zeros(1,1,C))
        self.k_a=nn.Parameter(torch.zeros(1,1,C)); self.r_k=nn.Parameter(torch.zeros(H,64))
        self.receptance=nn.Linear(C,C,bias=False); self.key=nn.Linear(C,C,bias=False)
        self.value=nn.Linear(C,C,bias=False); self.output=nn.Linear(C,C,bias=False)
        self.ln_x=nn.GroupNorm(H,C,eps=64e-5)
    def forward(self,x,v_first):
        B,T,C=x.shape; H=self.n_head; xx=F.pad(x[:,1:],(0,0,0,1))-x
        xr=x+xx*self.x_r; xw=x+xx*self.x_w; xk=x+xx*self.x_k
        xv=x+xx*self.x_v; xa=x+xx*self.x_a; xg=x+xx*self.x_g
        r=self.receptance(xr); w=-F.softplus(-(self.w0+torch.tanh(xw@self.w1)@self.w2))-0.5
        k=self.key(xk); v=self.value(xv)
        if self.lid==0: v_first=v
        else: v=v+(v_first-v)*torch.sigmoid(self.v0+(xv@self.v1)@self.v2)
        a=torch.sigmoid(self.a0+(xa@self.a1)@self.a2)
        g=torch.sigmoid(xg@self.g1)@self.g2
        kk=k*self.k_k; kk=F.normalize(kk.view(B,T,H,-1),dim=-1,p=2.0).view(B,T,C)
        k=k*(1+(a-1)*self.k_a)
        x=rwkv7_op(r,w,k,v,-kk,kk*a)
        x=self.ln_x(x.view(B*T,C)).view(B,T,C)
        x=x+((r.view(B,T,H,-1)*k.view(B,T,H,-1)*self.r_k).sum(-1,keepdim=True)*v.view(B,T,H,-1)).view(B,T,C)
        x=self.output(x*g); return x,v_first

class ChannelMix(nn.Module):
    def __init__(self, args): super().__init__(); self.x_k=nn.Parameter(torch.zeros(1,1,args.n_embd))
        self.key=nn.Linear(args.n_embd,args.dim_ffn,bias=False); self.value=nn.Linear(args.dim_ffn,args.n_embd,bias=False)
    def forward(self,x): xx=F.pad(x[:,1:],(0,0,0,1))-x; k=x+xx*self.x_k; k=torch.relu(self.key(k))**2; return self.value(k)

class Block(nn.Module):
    def __init__(self,args,lid): super().__init__(); self.lid=lid
        self.ln0=nn.LayerNorm(args.n_embd); self.ln1=nn.LayerNorm(args.n_embd); self.ln2=nn.LayerNorm(args.n_embd)
        self.att=TimeMix(args,lid); self.ffn=ChannelMix(args)
    def forward(self,x,vf):
        if self.lid==0: x=self.ln0(x)
        xx,vf=self.att(self.ln1(x),vf); x=x+xx; x=x+self.ffn(self.ln2(x)); return x,vf

class RWKV(nn.Module):
    def __init__(self,args): super().__init__()
        args.dim_att=args.n_embd; args.dim_ffn=args.n_embd*4
        self.emb=nn.Embedding(args.vocab_size,args.n_embd)
        self.blocks=nn.ModuleList([Block(args,i) for i in range(args.n_layer)])
        self.ln_out=nn.LayerNorm(args.n_embd); self.head=nn.Linear(args.n_embd,args.vocab_size,bias=False)
    def forward(self,idx,return_h=False):
        x=self.emb(idx); vf=torch.empty_like(x)
        for b in self.blocks: x,vf=b(x,vf)
        x=self.ln_out(x)
        if return_h: return self.head(x), x
        return self.head(x)

# ── Load ──
args=types.SimpleNamespace(vocab_size=65536,n_embd=768,n_layer=12,head_size_a=64)
model=RWKV(args).cuda()
sd=torch.load('rwkv7-g1d-0.1b-20260129-ctx8192.pth',map_location='cpu',weights_only=False)
for k,v in list(sd.items()):
    if isinstance(v,torch.Tensor) and v.dtype!=torch.float32: sd[k]=v.float()
model.load_state_dict(sd,strict=False)
model.eval(); [p.requires_grad_(False) for p in model.parameters()]
print(f'Model: {sum(p.numel()/1e6 for p in model.parameters()):.1f}M')

# ── Diffuser ──
class Diffuser(nn.Module):
    def __init__(self):
        super().__init__()
        self.norm=nn.LayerNorm(D*2)
        self.net=nn.Sequential(nn.Linear(D*2,D),nn.GELU(),nn.Linear(D,D))
        self.gate=nn.Parameter(torch.zeros(1))
    def forward(self,h,c): return h+torch.tanh(self.gate)*self.net(self.norm(torch.cat([h,c],-1)))

diff=Diffuser().cuda(); opt=torch.optim.AdamW(diff.parameters(),lr=3e-5)
ids=torch.from_numpy(np.load('checkpoints/mohe_fw_rwkv_1b.npy',mmap_mode='r'))

# ── Train ──
diff.train()
for bi in range(500):
    s=torch.randint(0,len(ids)-4*128,(1,)).item()
    x=ids[s:s+4*128].reshape(4,128).cuda()
    with torch.no_grad():
        logits,h=model(x,return_h=True)
    sigma=0.02+0.08*torch.rand(1).item()
    hn=h+torch.randn_like(h)*sigma
    hp=diff(hn.view(-1,D),(torch.softmax(logits*0.05,-1)@model.head.weight).view(-1,D))
    loss=F.mse_loss(hp,h.view(-1,D))
    opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(diff.parameters(),1.0); opt.step()
    torch.cuda.empty_cache()
torch.save({'diff':diff.state_dict()},'checkpoints/diff_12l.pt')

# ── Eval ──
diff.eval()
from rina.rwkv_tokenizer import TRIE_TOKENIZER
tok=TRIE_TOKENIZER('checkpoints/rwkv_vocab_v20230424.txt')
prompt='The Eiffel tower is in the city of'
p=torch.tensor([tok.encode(prompt)]).cuda()
for label,use_d,temp in [('AR',False,0.8),('AR+Diff',True,0.8)]:
    g=p.clone()
    with torch.no_grad():
        for _ in range(32):
            l,h=model(g,return_h=True)
            if use_d:
                hc=diff(h.view(-1,D),(torch.softmax(l*0.05,-1)@model.head.weight).view(-1,D)).view(1,-1,D)
                l=model.head(hc)
            probs=torch.softmax(l[:,-1]/temp,-1)
            g=torch.cat([g,torch.multinomial(probs,1)],1)
    text=tok.decode(g[0].tolist()[len(tok.encode(prompt)):])
    print(f'{label}: {repr(text)}')
print('Done.')
