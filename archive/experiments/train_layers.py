"""Train official RWKV-v7 with official fp32 CUDA kernel (WindBackstepping)."""
import sys, os; sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True'
import math, torch, torch.nn as nn, torch.nn.functional as F, numpy as np
from tqdm import tqdm
from rina.rwkv_v7_official import RWKV, args, HEAD_SIZE, CHUNK_LEN

device = 'cuda'
args.n_layer = 6
args.n_embd = 768
args.vocab_size = 65536

BSZ, SEQ = 8, 512
assert SEQ % CHUNK_LEN == 0  # kernel requires T % CHUNK_LEN == 0
LR = 3e-4; N_STEPS = 24000
DATA_PATH = 'checkpoints/mohe_fw_rwkv_1b.npy'
CKPT_DIR = 'checkpoints'
CSV_PATH = os.path.join(CKPT_DIR, 'train_official.csv')

model = RWKV(args).to(device)
nparam = sum(p.numel() for p in model.parameters())
print(f'Official RWKV-v7 (fp32 kernel): {nparam/1e6:.1f}M')

opt = torch.optim.AdamW(model.parameters(), lr=LR)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, N_STEPS)

ids = torch.from_numpy(np.load(DATA_PATH, mmap_mode='r'))
nb = (len(ids) - 1) // (BSZ * SEQ)

start_step = 0
resume_path = os.path.join(CKPT_DIR, 'official_latest.pt')
if os.path.exists(resume_path):
    ckpt = torch.load(resume_path, map_location='cpu', weights_only=False)
    model.load_state_dict(ckpt['model'])
    opt.load_state_dict(ckpt['opt'])
    start_step = ckpt['step']
    print(f'Resumed: {start_step}')

with open(CSV_PATH, 'a' if start_step > 0 else 'w', newline='') as f:
    if start_step == 0: f.write('step,loss,ppl,gn\n')

model.train()
pbar = tqdm(range(start_step, N_STEPS), initial=start_step, total=N_STEPS)
for bi in pbar:
    perm = torch.randperm(nb)
    s = perm[0] * BSZ * SEQ
    x = ids[s:s + BSZ * SEQ].view(BSZ, SEQ).to(device, dtype=torch.long)

    logits = model(x)
    loss = F.cross_entropy(logits[:, :-1].reshape(-1, args.vocab_size), x[:, 1:].reshape(-1))

    opt.zero_grad()
    loss.backward()
    gn = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0).item()
    opt.step()
    scheduler.step()

    if bi % 500 == 0: torch.cuda.empty_cache()
    if bi % 200 == 0:
        pbar.set_postfix(loss=f'{loss.item():.2f}', ppl=f'{math.exp(min(loss.item(),20)):.1f}', gn=f'{gn:.1f}')
    if bi % 1000 == 0 or bi == N_STEPS - 1:
        ppl = math.exp(min(loss.item(), 20))
        with open(CSV_PATH, 'a', newline='') as f:
            f.write(f'{bi+1},{loss.item():.6f},{ppl:.4f},{gn:.1f}\n')
        torch.save({'step': bi+1, 'model': model.state_dict(), 'opt': opt.state_dict()},
                   resume_path + '.tmp')
        os.replace(resume_path + '.tmp', resume_path)

torch.save({'step': N_STEPS, 'model': model.state_dict(), 'opt': opt.state_dict()},
           os.path.join(CKPT_DIR, 'official_final.pt'))
print(f'\nDone - official_final.pt')

# ==== Generation test ====
print('\n--- Greedy (24 tokens) ---')
model.eval()
prompt = torch.randint(10, args.vocab_size, (1, 4), device=device)
with torch.no_grad():
    gen = prompt.clone()
    for _ in range(20):
        logits = model(gen)
        nxt = logits[:, -1].float().argmax(-1, keepdim=True)
        gen = torch.cat([gen, nxt], dim=1)
    out = gen[0].tolist()
    uni = len(set(out))
    print(f'  Unique: {uni}/24  {"DIVERSE" if uni > 8 else "COLLAPSED"}')
print('Done.')
