...
BSZ, SEQ = 16, 256
LR = 3e-4
P1_STEPS, P2_STEPS = 10000, 10000
MASK_RATE = 0.7
DATA_PATH = 'checkpoints/mohe_fw_rwkv_1b.npy'
CKPT_DIR = 'checkpoints'
CSV_PATH_P1 = os.path.join(CKPT_DIR, 'p1_diffusion.csv')
CSV_PATH_P2 = os.path.join(CKPT_DIR, 'p2_info_conserve.csv')
...
with open(CSV_PATH_P1, 'w', newline='') as f:
    f.write('step,loss_cos,loss_norm,loss_ce,norm,gn\n')

print(f'\n{"="*50}\nPhase 1: DAE with cosine + CE regularization\n{"="*50}\n')
print(f'  Mask rate: {MASK_RATE}, per-token random replacement')
print(f'  Loss: cosine(h_pred, h_clean) + norm_reg + CE(h_pred_logits, x)\n')
...
for bi in pbar:
    ...
    emb_clean = gen.embed_norm(gen.embed(x))

    mask = torch.rand(BSZ, SEQ, device=device) < MASK_RATE
    rand_tokens = torch.randint(10, 65536, (BSZ, SEQ), device=device)
    x_corrupt = torch.where(mask, rand_tokens, x)
    emb_corrupt = gen.embed_norm(gen.embed(x_corrupt))

    sigma = 0.01 + 0.49 * 0.5 * (1 + math.cos(math.pi * bi / P1_STEPS))
    emb_corrupt += torch.randn_like(emb_corrupt) * sigma

    with torch.no_grad():
        h_clean = gen(emb=emb_clean)
    h_pred = gen(emb=emb_corrupt)
    logits = gen(h_pred, return_logits=True)

    h_f = h_pred.view(-1, DM)
    h_cf = h_clean.view(-1, DM)
    cos_sim = F.cosine_similarity(h_f, h_cf, dim=-1)
    loss_cos = (1 - cos_sim).mean()

    h_norm = h_pred.norm(dim=-1).mean()
    loss_norm = F.relu(0.3 - h_norm)

    loss_ce = F.cross_entropy(logits.view(-1, 65536), x.view(-1))

    loss = loss_cos + 0.1 * loss_norm + 1.0 * loss_ce
    ...
    if bi % 200 == 0:
        hn = h_pred.norm(dim=-1).mean().item()
        pbar.set_postfix(cos=f'{loss_cos.item():.4f}', ce=f'{loss_ce.item():.4f}',
                         norm=f'{hn:.3f}', gn=f'{gn:.1f}')
    if bi % 500 == 0 or bi == P1_STEPS - 1:
        with open(CSV_PATH_P1, 'a', newline='') as f:
            f.write(f'{bi+1},{loss_cos.item():.6f},{loss_norm.item():.6f},{loss_ce.item():.6f},{h_norm.item():.4f},{gn:.1f}\n')

torch.save({'gen': gen.state_dict()}, os.path.join(CKPT_DIR, 'p1_diffusion.pt'))
hs = h_pred.norm(dim=-1).mean().item()
print(f'Phase 1 done - p1_diffusion.pt (cos={loss_cos.item():.4f}, ce={loss_ce.item():.4f}, norm={hs:.3f})')

print(f'\n{"="*50}\nPhase 2: Info conservation (contrastive)\n{"="*50}\n')
with open(CSV_PATH_P2, 'w', newline='') as f:
...
for bi in pbar:
    ...
    if bi % 500 == 0 or bi == P2_STEPS - 1:
        with open(CSV_PATH_P2, 'a', newline='') as f:
...
print(f'All done - info_conserve_v3.pt')
