"""Minimal baseline: SSM + single SwiGLU FFN + CE. No MoE, no routing, no depth chain."""
import sys, os; sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True'
import math, torch, torch.nn as nn, torch.nn.functional as F, numpy as np
from tqdm import tqdm
from rina.model_minimal import MinimalModel
...
CSV_PATH = os.path.join(CKPT_DIR, 'train_minimal.csv')

model = MinimalModel(VOCAB, DM).to(device)
nparam = sum(p.numel() for p in model.parameters())
print(f'Minimal model: {nparam/1e6:.1f}M')
...
start_step = 0
resume_path = os.path.join(CKPT_DIR, 'minimal_latest.pt')
...
torch.save({'step': N_STEPS, 'model': model.state_dict(), 'opt': opt.state_dict()},
           os.path.join(CKPT_DIR, 'minimal_final.pt'))
print(f'\nDone - minimal_final.pt')
...
with torch.no_grad():
    ...
    if uni > 8:
        ...
        print(f'  Output: {out[:8]}...')
        print('  *** FAKE CONVERGENCE ***')
