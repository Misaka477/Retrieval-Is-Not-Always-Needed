"""RINA SH — 调度层硬路由版本: 只走调度头选的那条路，梯度干净"""
import math
...
class SHBlock(nn.Module):
    ...
    def forward(self,x,return_all=False):
        """return_all=True 时返回 (output, routes, all_losses) 用于训练"""
        ln=self.ln1(x)
        _,cq=self.l3(ln)
        route=F.softmax(self.scheduler(cq.detach()),dim=-1)
        # 按 batch 均值选路
        w=route.mean(dim=(0,1))
        choice=w.argmax().item()
        if return_all:
            # 训练时三路全算（只选一路 backward）
            out1=self.l1(ln)
            out2,_=self.l2(ln)
            out3,_=self.l3(ln)
            outs=[out1,out2,out3]
            return x+outs[choice]+self.mlp(self.ln2(x)), route, choice
        else:
            # 推理时只算选中的路
            if choice==0: out=self.l1(ln)
            elif choice==1: out,_=self.l2(ln)
            else: out,_=self.l3(ln)
            return x+out+self.mlp(self.ln2(x)), route

class RINA_SH(nn.Module):
    def __init__(self,c):
        super().__init__();self.config=c
        self.transformer=nn.ModuleDict(dict(
            wte=nn.Embedding(c.vocab_size,c.n_embd),
            drop=nn.Dropout(c.dropout),
            h=nn.ModuleList([SHBlock(c) for _ in range(c.n_layer)]),
        ...
        print(f'RINA_SH: {core/1e6:.2f}M core | {sum(p.numel() for p in self.parameters())/1e6:.2f}M total')
    def _iw(self,m):
    ...
    def forward(self,idx,targets=None,return_all=False):
        B,T=idx.size();assert T<=self.config.block_size
        x=self.transformer.drop(self.transformer.wte(idx))
        route_data=[]
        for b in self.transformer.h:
            if return_all:
                x,r,ch=b(x,return_all=True);route_data.append((r,ch))
            else:
                x,r=b(x);route_data.append(r)
        x=self.transformer.ln_f(x)
        if return_all:
            routes=torch.stack([r for r,_ in route_data],dim=2)
            choices=[ch for _,ch in route_data]
        else:
            routes=torch.stack(route_data,dim=2)
            choices=None
        if targets is not None:
            l=self.lm_head(x)
            ce=F.cross_entropy(l.view(-1,l.size(-1)),targets.reshape(-1),ignore_index=-1)
            if return_all:
                return l,ce,routes,choices
            return l,ce,routes
