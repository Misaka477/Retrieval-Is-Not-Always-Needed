"""Train Route C: Inertia Wave — 纯 O(T) 递推，无 attention"""
...
def train(cfg, data_path='data/english_pretrain.npy', lr=3e-4, bsz=4, steps=10000, out='checkpoints/rina_c'):
    ...
    model = RINA_C(cfg).to(device); model.train()
    ...
    for step in pbar:
        ...
        if step % 1000 == 0:
            ...
            with torch.no_grad():
                ...
                ce_v = val_loss.item()
            model.train()
            pbar.set_postfix(loss=f'{loss.item():.2f}', val=f'{ce_v:.2f}', lr=f'{sched.get_last_lr()[0]:.1e}')
            torch.save({'model': model.state_dict(), 'step': step}, f'{out}/rina_c_{step}.pt')

    torch.save({'model': model.state_dict()}, f'{out}/rina_c_final.pt')
...
if __name__ == '__main__':
    ...
    p.add_argument('--out', type=str, default='models/out-rina-c')
    p.add_argument('--dim', type=int, default=512); p.add_argument('--layers', type=int, default=12)
    p.add_argument('--heads', type=int, default=8); p.add_argument('--hdim', type=int, default=64)
    p.add_argument('--d-c', type=int, default=128)
    args = p.parse_args()
    cfg = RINA_C_Config(vocab_size=50257, block_size=args.seq, n_layer=args.layers,
                        n_head=args.heads, head_dim=args.hdim, n_embd=args.dim, d_c=args.d_c)
