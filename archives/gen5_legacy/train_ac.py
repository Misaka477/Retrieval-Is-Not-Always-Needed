"""Train Route AC: 浅层惯性波 + 深层注意力 + 稀疏索引"""
...
def train(cfg, data_path='data/english_pretrain.npy', lr=3e-4, bsz=4, steps=10000, out='checkpoints/rina_ac'):
    ...
    model = RINA_AC(cfg).to(device); model.train()
    ...
    for step in pbar:
        ...
        if step % 1000 == 0:
            ...
            torch.save({'model': model.state_dict(), 'step': step}, f'{out}/rina_ac_{step}.pt')

    torch.save({'model': model.state_dict()}, f'{out}/rina_ac_final.pt')
...
if __name__ == '__main__':
    ...
    p.add_argument('--sparse-k', type=int, default=8); p.add_argument('--sparse-w', type=int, default=4)
    args = p.parse_args()
    cfg = RINA_AC_Config(vocab_size=50257, block_size=args.seq, n_layer=args.layers,
                         n_head=args.heads, n_kv_heads=args.kv_heads, n_embd=args.dim, d_c=args.d_c,
                         use_int4=args.int4, sparse_k=args.sparse_k, sparse_local_w=args.sparse_w)
