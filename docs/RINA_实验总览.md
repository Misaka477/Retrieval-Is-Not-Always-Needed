...
- top-K 能定位 needle（NIAH 简单场景 12/12）
- 但**生成质量退化无法通过无训练方案解决**
...
|:-----|:---------|:----------|
| NIAH recall (gap≤32) | 100% | 100% |
| NIAH recall (gap=64) | 92% | ~10%（BPTT 梯度衰减） |
| 推理复杂度 | O(T²) | **O(T)** |
| 512K 推理可行 | ❌ impossible | ✅ ~8 分钟 |

**定论：** CANN-SSM 替换方向正确——recall 持平（gap≤32），复杂度从 O(T²) 降到 O(T)。长 gap 的 recall 差距由精确槽填补。
...
### 2.5 NIAH 完整实验矩阵（05-17~20）

#### 2.5.1 Toy NIAH

| gap | Hopfield+Slot | CANN-SSM V1 | SNN v2 |
|:---:|:-------------:|:-----------:|:------:|
| 8-32 | 100% | ~95-50% | **100%** |
| 64 | 92% | 16% | **100%** |
| 128 | — | 11% | **100%** |

#### 2.5.2 Real-text NIAH

15M LM 预训练后 post-hoc fine-tune，200-300 steps。

| gap | GPT-2 (固定位) | V1 CANN+slot | SNN v2 |
|:---:|:--------------:|:------------:|:------:|
| 8 | 100% | 22% | 23% |
| 128 | 100% | 22% | **32%** |

#### 2.5.3 Extreme NIAH（随机位置，gap=128）

| 模型 | recall | 显存 | 说明 |
|:-----|:------|:-----|:------|
| GPT-2 | **83%** | 3.0G | 固定位 100% → 随机位 83%（−17%），位置作弊被破 |
| V1 CANN+slot | 21% | 2.3G | 固定/随机位不变（21%→21%）|
| **SNN v2** | **100%** | — | 内容寻址不受位置影响 |

#### 2.5.4 Multi-key NIAH（3 key, gap=128）🔥

| 模型 | 1 key | 3 keys | Δ |
|:-----|:-----|:-------|:---|
| GPT-2 | 83% | 36% | **−47%** |
| V1 CANN+slot | 21% | 18% | −3% |
| **SNN v2** | **21%** | **100%** | **+79%** |

**三条核心结论：**
1. **GPT-2 O(T²) attention 多 needle 交叉串扰** — 每增一个 needle 信号稀释
2. **V1 slot 独立读不竞争** — 但 gate 过度依赖每步 attractor
3. **SNN v2 temporal sparsity 让 gate 学会独立维持状态** — slot 注入作为新鲜信号被主动接收
...
*整理自 `RINA实验日志.md`（4274 行, 2026-05-15~05-21）*
