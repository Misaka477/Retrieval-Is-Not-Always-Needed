...
static void dump(const char* name, const float* d, int n) {
    ...
    char path[256]; snprintf(path,256,"/tmp/j_%s",name);
...
int main() {
    ...
    dump("l0_h_embed",bufs.fwd.h,n*d);
    for(int l=0;l<1;l++){  // just layer 0 (SSM)
    ...
    dump("l0_h_out",bufs.fwd.h,n*d);
    fprintf(stderr,"layer 0 done, err=%s\n",cudaGetErrorString(cudaGetLastError()));
