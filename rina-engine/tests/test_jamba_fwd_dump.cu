#include <cuda_runtime.h>
#include <cstdio>
#include <cstdlib>
#include <vector>
#include "core/config.h"
#include "core/tensor.h"
#include "core/layer.h"
#include "core/buffer.h"
#include "training/train.h"
#include "model.h"

static void dump(const char* name, const float* d, int n) {
    std::vector<float> cpu(n);
    cudaMemcpy(cpu.data(), d, n*sizeof(float), cudaMemcpyDeviceToHost);
    char path[256]; snprintf(path,256,"/tmp/fwd_%s",name);
    FILE*f=fopen(path,"wb"); fwrite(cpu.data(),sizeof(float),n,f); fclose(f);
}

extern void launch_embedding_fp32(const float*,const int*,float*,int,int,int,cudaStream_t);

int main() {
    ModelConfig cfg; TensorMap w;
    if(!load_model("/tmp/jamba_qw2.rinn",cfg,w)){fprintf(stderr,"load fail\n");return 1;}
    auto layers=build_layers(cfg,w);
    int B=1,T=3,n=B*T,d=cfg.dim,V=cfg.vocab_size;
    int H=cfg.n_heads, dh=cfg.head_dim, Hkv=cfg.n_kv_heads, dc=cfg.d_c, ss=cfg.ssm_steps, hd=d*4*2/3/256*256;
    int ws=0,total=0;
    for(auto& l:layers){int w=l->workspace_per_token(d,H,dh);if(w>ws)ws=w;total+=l->saved_per_token(d,H,dh);}
    BufferManager bufs;
    bufs.alloc_fwd(n,d,ws,hd,V,total);
    cudaStream_t s;cudaStreamCreate(&s);
    int*d_ids;cudaMalloc(&d_ids,n*sizeof(int));
    int ids[]={128000,9906,11};
    cudaMemcpy(d_ids,ids,n*sizeof(int),cudaMemcpyHostToDevice);
    const float*wte=(const float*)w.get("transformer.wte.weight")->data;
    launch_embedding_fp32(wte,d_ids,bufs.fwd.h,B,T,d,s);
    cudaStreamSynchronize(s);
    dump("embed",bufs.fwd.h,n*d);

    // Run layer 0 (SSM) via engine forward
    float*base_save=bufs.fwd.save;
    bufs.fwd.save=base_save+layers[0]->save_offset*n;
    layers[0]->forward(bufs.fwd.h,bufs.fwd,B,T,s);
    cudaStreamSynchronize(s);
    bufs.fwd.save=base_save;
    dump("h_after_l0",bufs.fwd.h,n*d);
    // Save buffer has: h_in, cq, db, mems, ds, ln2_in, ln2_out, gu
    int n_saved = layers[0]->saved_per_token(d,H,dh);
    dump("save_l0",base_save+layers[0]->save_offset*n,n_saved*n);

    // Also dump bufs.a and bufs.m for debug
    dump("buf_a",bufs.fwd.a,n*d);
    int fw=n*std::max(d,hd);
    dump("buf_m",bufs.fwd.m,n*ws);

    cudaError_t e=cudaGetLastError();
    if(e!=cudaSuccess)fprintf(stderr,"err: %s\n",cudaGetErrorString(e));
    fprintf(stderr,"done\n");
    w.free_all();bufs.free_all();cudaFree(d_ids);cudaStreamDestroy(s);
    return 0;
}
