...
struct MLAImpl {
    ...
    const float *w_dqkv,*q_norm_w,*w_uq,*w_uk,*w_k2v,*w_qr,*w_kr,*c_proj;
    ...
    bool init(const ModelConfig& cfg,const TensorMap& w,int l) {
        ...
        return (w_dqkv&&w_uq&&c_proj&&w1&&w2&&w3&&ln1_w&&ln2_w);
    }
    ...
    void forward(float*h,ForwardBuffers&b,int B,int T,cudaStream_t s) {
        ...
        if(rqc)launch_rope_fp32(b.m+oqr,rqc,rqs,B,T,H,dhr,s);
        if(rc)launch_rope_fp32(b.m+okr,rc,rs,B,T,Hkv,dhr,s);
    ...
    void backward(GradBuffers& g,ForwardBuffers&b,float*wg,int B,int T,cudaStream_t s) {
        ...
        if(rqc)launch_rope_bwd_fp32(dqr_,dqr_,rqc,rqs,B,T,H,dhr,s);
        if(rc)launch_rope_bwd_fp32(dkr_,dkr_,rc,rs,B,T,Hkv,dhr,s);
...
extern "C" {
static bool mla_init(void*s,const ModelConfig&c,const TensorMap&w,int l){return((MLAImpl*)s)->init(c,w,l);}
static void mla_fwd(void*s,float*h,ForwardBuffers&b,int B,int T,cudaStream_t st){((MLAImpl*)s)->forward(h,b,B,T,st);}
static void mla_bwd(void*s,GradBuffers&g,ForwardBuffers&b,float*wg,int B,int T,cudaStream_t st){((MLAImpl*)s)->backward(g,b,wg,B,T,st);}
static int mla_ws(void*s,int d,int h,int hd){return((MLAImpl*)s)->ws_per_token();}
static int mla_sv(void*s,int d,int h,int hd){return((MLAImpl*)s)->sv_per_token();}
static void mla_del(void*s){delete(MLAImpl*)s;}
}
static const LayerVTable mla_vtab={mla_init,mla_fwd,mla_bwd,mla_ws,mla_sv,mla_del};
Layer create_mla_layer(){Layer l;l.impl=new MLAImpl();l.vtab=&mla_vtab;return l;}
