...
// Forward declarations for fused kernel (defined below)
...
void launch_linear_dispatch(
    ...
    switch (quant_type) {
        ...
        case QuantType::GGML_IQ3_XXS:
        case QuantType::GGML_IQ3_S:
...
// ─── Fused Q2_K vec×matmul for M=1 (avoids materializing fp32 weight) ───
// output[N] = input[K] @ weight[N,K]_Q2K
// One block per output element, shared memory reduction
static inline __device__ float ggml_half_to_float_fused(uint16_t h) {
    return __half2float(__ushort_as_half(h));
}
__global__ void q2k_vec_mul_kernel(const float* __restrict__ input, const void* __restrict__ weight,
    float* __restrict__ output, int N, int K) {
    ...
    for (int i = tid; i < K; i += blockDim.x) {
        ...
        float dall = ggml_half_to_float_fused(*(const uint16_t*)(blk_data + 80));
        float dmin = ggml_half_to_float_fused(*(const uint16_t*)(blk_data + 82));
        int page = in_blk / 128;
        int pos = in_blk % 128;
        int l = pos % 32;
...
// Fallback Q2_K vec×matmul for M>1: process one output element per block
__global__ void q2k_vec_mul_m_kernel(const float* __restrict__ input, const void* __restrict__ weight,
    ...
    for (int i = tid; i < K; i += blockDim.x) {
        ...
        float scale_val = dall * (sc & 0xF);
        float min_val = dmin * (sc >> 4);
        int q = (blk_data[16 + page * 32 + l] >> (group_of_32 * 2)) & 3;
        float deq = scale_val * q - min_val;
        sum += input[(size_t)m * K + i] * deq;
    }
    __shared__ float sh[256];
    sh[tid] = sum;
    __syncthreads();
    ...
    if (tid == 0) output[(size_t)m * N + n] = sh[0];
}
