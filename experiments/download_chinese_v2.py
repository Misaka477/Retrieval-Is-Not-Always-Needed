"""下载 FineWeb-2 中文，批量 tokenize，跳过已有"""
...
OUT_NPY = f'data/chinese_pretrain_llama_{n_m}m.npy'

EXISTING = 0
if os.path.exists('data/chinese_pretrain_llama.npy'):
    EXISTING = np.load('data/chinese_pretrain_llama.npy', mmap_mode='r').shape[0]
...
AVG_TOK_PER_DOC = 1500
SKIP_DOCS = EXISTING // AVG_TOK_PER_DOC
print(f'跳过 ~{SKIP_DOCS} 个文档')
ds = load_dataset('TiWu-Lab/fineweb-2-zh', split='train', streaming=True).skip(SKIP_DOCS)
...
for i, ex in enumerate(ds):
    ...
    if col - tmp_idx * CHUNK >= CHUNK:
        np.save(f'data/_chinese_tmp_{tmp_idx}.npy', np.array(buf, dtype=np.int32))
        tmp_idx += 1; buf = []
...
if buf:
    np.save(f'data/_chinese_tmp_{tmp_idx}.npy', np.array(buf, dtype=np.int32)); tmp_idx += 1
print(f'Merging {tmp_idx} chunks...')
parts = [np.load(f'data/_chinese_tmp_{i}.npy', mmap_mode='r') for i in range(tmp_idx)]
np.save(OUT_NPY, np.concatenate(parts)[:TARGET])
for i in range(tmp_idx): os.remove(f'data/_chinese_tmp_{i}.npy')
