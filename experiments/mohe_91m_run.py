"""
MoHE 91M on FineWeb + StarCoder (200M tokens). 
...
MAX_DEPTH = 3
CKPT_DIR = "../checkpoints"; CKPT_NAME = "mohe_91m"
...
model = MoHE(VOCAB, DM, NP, n_experts=4,
             aux_loss_weight=0.5, route_noise=0.2, topk=2).to(device)
...
weights_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), CKPT_DIR, "mohe_74m_weights.pt")
if os.path.exists(resume_path):
    ...
    for k in list(sd.keys()):
        if k.startswith("prev_route") or "_batch_" in k:
            del sd[k]
    model.load_state_dict(sd, strict=False)
    print(f"  Loaded mohe_74m_weights.pt, proj layers random init")

LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), CKPT_DIR, f"{CKPT_NAME}.csv")
print("Training MoHE 91M on FineWeb+StarCoder+OpenWebMath...")
global_step = 0
for ep in range(start_ep, EPOCHS + 1):
    model.train(); total_loss = 0.0
    perm = torch.randperm(len(ids) - BS * SEQ)
    its = nb // SUBSAMPLE
    pbar = tqdm(range(min(its, STEPS_PER_EPOCH)), desc=f"ep {ep}/{EPOCHS}")
    for bi in pbar:
        global_step += 1
        start = perm[torch.randint(0, len(perm) - BS * SEQ, (1,)).item()]
