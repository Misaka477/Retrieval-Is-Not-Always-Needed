"""Expiatio Phase 1: Token 恒等性（加载 L1 权重，加 L2 loss）"""
...
def token_loss(states, token_ids, tau=0.5, max_pairs=512):
    """同一个 token 的状态应靠近。批量版。"""
    B, T, D = states.shape; h = states.view(-1, D); ids = token_ids.view(-1)
    unique_ids = ids.unique()
    anchors, positives = [], []
    for tid in unique_ids:
        positions = torch.where(ids == tid)[0]
        if positions.size(0) < 2: continue
        if len(anchors) >= max_pairs: break
        idx = torch.randperm(positions.size(0))[:2]
        anchors.append(h[positions[idx[0]].item()])
        positives.append(h[positions[idx[1]].item()])
    if len(anchors) < 2: return torch.tensor(0.0, device=states.device), 0.0, 0.0, 0.0
    
    a = F.normalize(torch.stack(anchors), dim=-1)
    p = F.normalize(torch.stack(positives), dim=-1)
    
    # 负样本: 所有其他 anchor 互为负样本
    pos_logits = (a * p).sum(-1) / tau  # [M]
    neg_logits = a @ p.T / tau  # [M, M] — other pairs serve as negatives
    
    # Mask out diagonal (positive pairs)
    M = a.size(0)
    mask = ~torch.eye(M, dtype=torch.bool, device=states.device)
    neg_logits = neg_logits[mask].view(M, M-1)  # [M, M-1]
    
    logits = torch.cat([pos_logits.unsqueeze(-1), neg_logits], dim=-1)  # [M, M]
    labels = torch.zeros(M, dtype=torch.long, device=states.device)
    
    loss = F.cross_entropy(logits, labels)
    with torch.no_grad():
        acc = (logits.argmax(-1) == labels).float().mean().item()
        pd = (a - p).norm(dim=-1).mean().item()
        # nd: all cross-pair distances
        nd = (a.unsqueeze(1) - a.unsqueeze(0)).norm(dim=-1)[mask].mean().item()
    return loss, acc, pd, nd
...
model.load_state_dict(ckpt['model']); print(f'Loaded L1 weights')
...
], lr=1e-4, betas=(0.9, 0.95))  # 更低的学习率，不破坏 L1 结构

N_STEPS = 10000
CSV_PATH = os.path.join(CKPT_DIR, 'p1_log.csv')
with open(CSV_PATH, 'w') as f:
    f.write('step,l1_loss,l2_loss,u_loss,total,l1_acc,tok_acc,pd_nd,t_pd,t_nd,ratio,lr\n')

model.train(); pbar = tqdm(range(N_STEPS)); t0 = time.time()
for step in pbar:
    x = get_batch(); states = model(x)
    tau = max(0.2, 0.8 * (1 - step / N_STEPS))
    l1, l1_acc, l1_pd, l1_nd, gap = contrastive_loss(states, tau=tau)
    u = vicreg_loss(states)
    t_loss, t_acc, t_pd, t_nd = token_loss(states, x, tau=tau)
    loss = l1 + 0.5 * u + 0.3 * t_loss
    ...
    if step % 500 == 0:
        ...
        model.train()
        pbar.set_postfix(l1=f'{l1.item():.2f}', l2=f'{t_loss:.2f}', u=f'{u.item():.3f}',
                         l1a=f'{l1_acc:.2f}', tka=f'{t_acc:.2f}', ratio=f'{ratio:.4f}')
        with open(CSV_PATH, 'a') as f:
            f.write(f'{step},{l1.item():.4f},{t_loss:.4f},{u.item():.6f},{loss.item():.4f},{l1_acc:.4f},{t_acc:.4f},{l1_pd:.4f},{t_pd:.4f},{t_nd:.4f},{ratio:.4f},{opt.param_groups[0]["lr"]:.2e}\n')
        torch.save({'model': model.state_dict(), 'step': step}, os.path.join(CKPT_DIR, f'p1_{step}.pt'))

print(f'Done in {(time.time()-t0)/60:.1f}min')
torch.save({'model': model.state_dict()}, os.path.join(CKPT_DIR, 'p1_final.pt'))
