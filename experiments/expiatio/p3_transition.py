"""Expiatio Phase 3: Transition 建模 — 预测 h_{t+1} 来自 h_t。"""
...
class TransitionPredictor(nn.Module):
    """轻量 MLP: h_t → h_{t+1}_pred。很小，不会过拟合。"""
...
# Transition 预测器（单独训练，不收 backbone 梯度）
predictor = TransitionPredictor().to(device)
print(f'Predictor: {sum(p.numel() for p in predictor.parameters())/1e3:.1f}K')

# 冻结 backbone，只训 predictor
for p in model.parameters(): p.requires_grad_(False)
model.eval()

opt_p = torch.optim.AdamW(predictor.parameters(), lr=1e-3)

N_STEPS = 10000
CSV_PATH = os.path.join(CKPT_DIR, 'p3_trans_log.csv')
with open(CSV_PATH, 'w') as f:
    f.write('step,t_loss,t_cos,l1_ratio,l1_pd,l1_nd\n')

predictor.train(); pbar = tqdm(range(N_STEPS)); t0 = time.time()
for step in pbar:
    x = get_batch()
    with torch.no_grad():
        states = model(x)  # backbone 冻结
    
    h_t = states[:, :-1]  # [B, T-1, D]
    ...
    if step % 1000 == 0:
        ...
        with torch.no_grad():
            ...
            _, _, pd, nd, _ = contrastive_loss(sv)
        predictor.train()
        pbar.set_postfix(tl=f'{loss.item():.4f}', tcos=f'{t_cos:.3f}',
                         ratio=f'{ratio:.4f}', pd=f'{pd:.3f}', nd=f'{nd:.3f}')
        with open(CSV_PATH, 'a') as f:
            f.write(f'{step},{loss.item():.6f},{t_cos:.4f},{ratio:.4f},{pd:.4f},{nd:.4f}\n')
        torch.save({'predictor': predictor.state_dict(), 'step': step}, os.path.join(CKPT_DIR, f'p3_{step}.pt'))

print(f'\nDone in {(time.time()-t0)/60:.1f}min')
torch.save({'predictor': predictor.state_dict()}, os.path.join(CKPT_DIR, 'p3_final.pt'))
