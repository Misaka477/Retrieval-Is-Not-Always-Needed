...
class PyramidAttention(nn.Module):
    ...
    def _coarse_attention(self, q_t, k_t, stride, max_k, causal, T):
        ...
        sc = sc + causal_c
        # 结果映射回原始索引
        scores_full = torch.zeros(B, H, Tq, T, device=q_t.device)
        scores_full[:, :, :, ::stride] = sc  # 只填充降采样的位置
        # 把 causal mask 也恢复
        mask = torch.zeros(B, H, Tq, T, device=q_t.device)
        mask[:, :, :, ::stride] = 1
        scores_full = scores_full.masked_fill(mask == 0, float('-inf'))
        # 基于 Q 范数做自适应: 范数高的 query 需要更大的 k
        q_norm = q_t.norm(dim=-1)  # [B, H, T]
        # 自适应 k: 高范数 → 低信息量 → 用更大的 k
        # 按范数的分位数分配 k
        sorted_norms = q_norm.sort(dim=-1).values
        thresholds = sorted_norms[:, :, T // 4 * 3]  # 前 25% 位置用大 k
