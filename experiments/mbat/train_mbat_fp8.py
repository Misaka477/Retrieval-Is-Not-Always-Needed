"""MBAT-fp8: DeepSeek 式 fp8 训练（权重存 fp8，计算用 bf16）"""
...
SEQ, BSZ, LR = 512, 8, 3e-4; N_STEPS = 200000
...
class FP8Linear(nn.Module):
    """DeepSeek 式 fp8: master 存 fp32，前向量化到 fp8 再升回 bf16 计算"""
    def __init__(self, in_f, out_f, bias=False):
        super().__init__()
        self.weight = nn.Parameter(torch.empty(out_f, in_f))  # fp32 master copy
        self.bias = nn.Parameter(torch.zeros(out_f)) if bias else None
        nn.init.normal_(self.weight, 0, 0.02)

    def forward(self, x):
        if self.training and torch.cuda.get_device_capability() >= (8, 9):
            # 前向量化到 fp8，再升回 bf16 计算（模仿 fp8 存储效果）
...
class Attention(nn.Module):
    def __init__(self, max_k=64, lt='cbtka'):
        ...
        self.W_DKV = nn.ModuleList([FP8Linear(DM, d_c_g) for _ in range(self.G)])
        self.KV_norm = nn.ModuleList([nn.LayerNorm(d_c_g) for _ in range(self.G)])
        self.W_UK = nn.ModuleList([FP8Linear(d_c_g, DM // self.G) for _ in range(self.G)])
        self.W_UV = nn.ModuleList([FP8Linear(d_c_g, DM // self.G) for _ in range(self.G)])
        self.W_DQ = FP8Linear(DM, d_c_q); self.Q_norm = nn.LayerNorm(d_c_q)
        self.W_UQ = FP8Linear(d_c_q, DM)
        self.W_QR = FP8Linear(d_c_q, d_h_R * N_HEADS)
        self.W_KR = FP8Linear(DM, d_h_R); self.W_O = FP8Linear(DM, DM)
    ...
    def forward(self, x):
        ...
        if self.lt == 'window':
            ...
            return self.W_O((F.softmax(sc, -1).unsqueeze(-1) * v_tk).sum(3).transpose(1, 2).contiguous().view(B, T, -1))
...
class Block(nn.Module):
    def __init__(self, i):
        ...
        self.ffn = nn.Sequential(FP8Linear(DM, DM*4), nn.GELU(), FP8Linear(DM*4, DM))
...
class MBATfp8(nn.Module):
    def __init__(self):
        ...
        self.ln = nn.LayerNorm(DM); self.head = FP8Linear(DM, VOCAB)
        self.apply(self._init)
    def _init(self, m):
        if isinstance(m, FP8Linear): nn.init.normal_(m.weight, 0, 0.02)
    def forward(self, x):
        h = self.emb(x)
