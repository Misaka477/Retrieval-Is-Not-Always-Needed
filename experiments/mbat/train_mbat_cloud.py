...
CSV_PATH = os.path.join(CKPT_DIR, 'mbat_cloud_log.csv')
    ...
    def _pyramid_coarse(self, q_t, k_t, k_top, T):
        ...
        for b in range(B):
            for h in range(H):
                ...
                # Causal mask: query at pos i sees keys at j*2 <= i
                q_idx = mid_pos.float()  # [M]
                k_idx = torch.arange(0, T, 2, device=q_t.device).float()  # [kM]
                cm = (q_idx[:, None] >= k_idx[None, :]).float()  # [M, kM]
                cm = cm.masked_fill(cm == 0, float('-inf'))
                sc_mid = sc_mid + cm.unsqueeze(0).unsqueeze(0)
...
# 加载本地预训练权重（SEQ=128 训出的），做初始化
init_ckpt = os.path.join(CKPT_DIR, 'mbat_v4_final.pt')
if os.path.exists(init_ckpt):
    sd = torch.load(init_ckpt, map_location=device)
    sd = sd['model'] if 'model' in sd else sd
    # 过滤掉尺寸不匹配的键（cos/sin buffer 因 SEQ 不同而不同）
    sd_fixed = {}
    model_sd = model.state_dict()
    for k, v in sd.items():
        if k in model_sd and v.shape == model_sd[k].shape:
            sd_fixed[k] = v
    msg = model.load_state_dict(sd_fixed, strict=False)
    print(f'Loaded init weights from {init_ckpt}: {len(sd_fixed)}/{len(sd)} keys matched')
t = sum(p.numel() for p in model.parameters())
print(f'Params: {t/1e6:.2f}M')
opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=0.01)
sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, N_STEPS)
start_step = 0
resume_ckpt = os.path.join(CKPT_DIR, 'mbat_v4.pt')
if os.path.exists(resume_ckpt):
    ...
    if 'opt' in ck:
        ...
        print(f'Resumed from step {ck["step"]} (full state)')
    else:
        print(f'Resumed from step {ck["step"]} (model only, opt/sched reset)')
    start_step = ck['step'] + 1
model.train()
if not os.path.exists(CSV_PATH):
    with open(CSV_PATH, 'w') as f: f.write('step,loss,val_ce,ppl,lr\n')

pbar = tqdm(range(start_step, N_STEPS), initial=start_step, total=N_STEPS); t0 = time.time()
for step in pbar:
    x = get_batch(BSZ, SEQ)
    ...
    if step % 1000 == 0:
        ...
        torch.save({'model': model.state_dict(), 'opt': opt.state_dict(), 'sched': sched.state_dict(), 'step': step}, os.path.join(CKPT_DIR, 'mbat_v4.pt'))
        if ce_v < BEST_VAL_CE:
