"""
16M MoHE-Hybrid (independent h_i + lightweight GQA attention).
DM=256, NP=512, FineWeb, 3000 steps.
Verifies attention provides induction head lift over attractor-only.
"""
import sys, os; sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
import torch, torch.nn as nn, torch.nn.functional as F, numpy as np
from tqdm import tqdm
from rina.architectures import MoHEHybrid
...
LR = 3e-4
N_STEPS = 3000
LOG_PATH = 'checkpoints/mohe_hybrid_16m.csv'
WEIGHTS_PATH = 'checkpoints/mohe_hybrid_16m.pt'

# Load or train
if os.path.exists(WEIGHTS_PATH):
    print('Loading saved weights...')
    model = MoHEHybrid(VOCAB, DM, NP, n_experts=4, aux_loss_weight=0.5, route_noise=0.2, topk=2).to(device)
    ...
    print(f'  Attn params: {sum(p.numel() for p in model.attn.parameters())/1e6:.4f}M')
    ...
    pbar = tqdm(range(min(nb, N_STEPS)), desc='MoHE-Hybrid 16M')
    for bi in pbar:
        ...
        if bi % 200 == 199 or bi == min(nb, N_STEPS)-1:
            ...
            gr = getattr(model, '_last_gate_ratio', 0.0)
            es = getattr(model, '_last_exp_sim', 0.0)
            aux = getattr(model, '_last_aux_loss', 0.0)
            lr_now = float(opt.param_groups[0]['lr'])
            with open(LOG_PATH, 'a', newline='') as f:
                f.write(f'{bi+1},{ppl:.1f},{loss.item():.2f},{lr_now:.2e},{es:.4f},{gr:.4f},{gn:.4f},{aux:.6f}\n')
            pbar.set_postfix(ppl=f'{ppl:.1f}', gn=f'{gn:.3f}', sim=f'{es:.3f}')
