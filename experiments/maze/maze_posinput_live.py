"""Watch GRU+position model navigate a maze."""
...
ckpt=torch.load(os.path.join(BASE_DIR,'checkpoints','maze_posinput.pt'),weights_only=False,map_location=DEVICE)
...
for step in range(200):
    ...
    with torch.no_grad():
        for a in acts:
            pred,_ = model(ot,torch.tensor([[a]],device=DEVICE),model_state)
            # Extract predicted position (last 2 dims of pred obs)
            py,px = pred[0,-2].item()*h, pred[0,-1].item()*w  # y*xh, x*xw
            ny,nx = int(round(py)), int(round(px))
            ny,nx = max(0,min(h-1,ny)), max(0,min(w-1,nx))
            
            if maze[ny,nx]==CELL_EXIT: score=1000
            else: score=-abs(ny-EXIT[0])-abs(nx-EXIT[1])
    ...
    dy2,dx2=DIRS[facing]
    ax.arrow(pos[1],pos[0],dx2*0.3,dy2*0.3,head_width=0.3,head_length=0.3,fc='#ff6b6b',ec='#ff6b6b',zorder=6)
    ...
    ax.set_title(f'GRU+position input  |  Step {step+1}  |  Dist: {dist}',fontsize=11)
